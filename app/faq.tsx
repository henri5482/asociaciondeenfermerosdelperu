    ...
    <section className="w-full px-4 sm:px-6 lg:px-8 py-16 md:py-24 bg-[#0f1e26] text-gray-50 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto relative px-4 z-10">
        ...
        >
                ...
                >
                  ...
                  >
                    ...
                    <CardHeader className="flex flex-row items-start gap-3 p-0 pb-4 sm:gap-4 sm:pb-5">
                      ...
                      <Avatar className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 border-2 border-[#00c2a8] flex-shrink-0 shadow-md"> {/* Reduced border-width for smaller avatars */}
                        <AvatarImage src={testimonial.avatar} alt={testimonial.name} />
                        <AvatarFallback className="bg-[#00c2a8]/20 text-[#00c2a8] font-bold text-lg sm:text-xl">{testimonial.name.charAt(0)}</AvatarFallback> {/* Responsive fallback text size */}
                      </Avatar>
                      <div className="flex-1">
                        {/* Responsive title size: text-lg on mobile, larger on sm/lg */}
                        <CardTitle className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-100 mb-0 sm:mb-1">{testimonial.name}</CardTitle> {/* Reduced margin-bottom */}
                        ...
                        <div className="flex items-center gap-0.5 mt-1 sm:mt-2"> {/* Reduced gap and margin-top */}
                          {[...Array(5)].map((_, i) => (
                            <StarIcon
                              key={i}
                    ...
                    <CardContent className="p-0 text-sm sm:text-base leading-relaxed flex-grow overflow-hidden pt-3"> {/* Added small padding top to separate from header slightly */}
                      <blockquote className="italic text-gray-300 pl-3 border-l-3 border-[#00c2a8] text-sm sm:text-base"> {/* Reduced border-left width and padding-left */}
                        &quot;{testimonial.comment}&quot;
                      </blockquote>
                    </CardContent>
                  </Card>
          ...
          {/* Navigation controls */}
          <div className="absolute top-1/2 -translate-y-1/2 -left-3 sm:-left-6 right-0 flex justify-between px-2 sm:px-4 pointer-events-none z-20">
